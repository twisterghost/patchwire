...
class ClientManager {
  ...
  addClient (client) {
    ...
    client.on('close', () => {
      this.fire('clientDropped', {client, reason: 'close'});
      this.removeClient(client.clientId);
    });

    client.on('error', () => {
      this.fire('clientDropped', {client, reason: 'error'});
      this.removeClient(client.clientId);
    });
    ...
    client.on('disconnect', () => dc('disconnect'));
    client.on('connection_error', () => dc('connection_error'));
    client.on('connection_timeout', () => dc('connection_timeout'));
    client.on('end', () => dc('end'));
  ...
  /**
   * Disconnects a client and removes it from the client manager
   * @param {Client}  The client object to disconnect
   * @param <optional> {String} The reason for disconnecting
   */
  disconnectClient (client, reason = '') {
    this.fire('clientDropped', { client, reason });
    this.removeClient(client.clientId);
    client.disconnect();
  }
  ...
  handleIncomingCommand (client, data) {
    ...
    if (this.commandHandlers[data.command]) {
